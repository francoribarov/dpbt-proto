import { Clipboard, Download, ExternalLink, Sparkles, Stethoscope, X } from 'lucide-react'
import { useEffect, useMemo, useState } from 'react'
import { useNavigate } from 'react-router-dom'
import { AppBar, Card, InfoBox, PrimaryButton, SecondaryButton } from '../components/ui'
...
export const Summary = () => {
  ...
  const [summaryText, setSummaryText] = useState(
    draft.summaryText ?? buildSummaryText({ ...draft, triage: triage.level }, triage),
  )
  ...
  useEffect(() => {
    if (draft.summaryText) {
      ...
      setSummaryText(buildSummaryText({ ...draft, triage: triage.level }, triage))
    }
  }, [draft, triage])
  ...
  const recommendations = useMemo(
    () =>
      ...
      (draft.symptom ? buildRecommendations(draft.symptom, severityLevel) : null),
    ...
    <div className="space-y-6">
      ...
      <Card className="p-5 space-y-4">
        <div className="flex items-start justify-between gap-2">
          <div>
            <div className="text-sm font-semibold text-gray-900">Recomendaciones iniciales</div>
            <div className="text-xs text-gray-600">No sustituyen una consulta veterinaria</div>
          </div>
          <Stethoscope className="h-5 w-5 text-gray-600" />
        </div>
        {recommendations?.medicacion?.some((m) => m.includes('⚠️')) && (
          <InfoBox>{recommendations.medicacion.find((m) => m.includes('⚠️'))}</InfoBox>
        )}
        <div className="grid gap-3 md:grid-cols-3">
          <div className="rounded-2xl border border-gray-200 bg-gray-50 p-3">
            <div className="mb-2 text-sm font-semibold text-gray-800">Cuidados en casa</div>
            <ul className="space-y-2 text-sm text-gray-700">
              {(recommendations?.cuidados ?? []).filter(Boolean).map((item, idx) => (
                <li key={idx}>• {item}</li>
              ))}
            </ul>
          </div>
          <div className="rounded-2xl border border-gray-200 bg-gray-50 p-3">
            <div className="mb-2 text-sm font-semibold text-gray-800">Medicación sugerida (prototipo)</div>
            <ul className="space-y-2 text-sm text-gray-700">
              {(recommendations?.medicacion ?? []).filter(Boolean).map((item, idx) => (
                <li key={idx}>• {item}</li>
              ))}
            </ul>
          </div>
          <div className="rounded-2xl border border-gray-200 bg-gray-50 p-3">
            <div className="mb-2 text-sm font-semibold text-gray-800">Cuándo consultar urgente</div>
            <ul className="space-y-2 text-sm text-gray-700">
              {(recommendations?.urgente ?? []).filter(Boolean).map((item, idx) => (
                <li key={idx}>• {item}</li>
              ))}
            </ul>
          </div>
        ...
        <div className="fixed inset-0 z-40 grid place-items-center bg-black/30 px-4 py-6">
          <div className="max-h-[90vh] w-full max-w-md overflow-y-auto rounded-2xl border border-gray-200 bg-white p-5 shadow-2xl">
            <div className="mb-4 flex items-center justify-between">
              <div>
                <div className="text-sm font-semibold text-gray-800">Agenda simulada</div>
                <div className="text-xs text-gray-500">
                  Opciones falsas para testear preferencia de horario
                </div>
              </div>
              <button onClick={() => setSlotModal(false)} className="text-gray-500 hover:text-gray-700">
                <X className="h-5 w-5" />
              </button>
            </div>
